export const QUESTIONS = [
  {
    text: "По якій ноті музиканти налаштовують свої інструменти?",
    options: [
      {
        text: "Мі",
        isTrue: false
      },
      {
        text: "Фа",
        isTrue: true
      },
      {
        text: "Соль",
        isTrue: false
      },
      {
        text: "Сі",
        isTrue: false
      }
    ]
  },
  {
    text: 'Яке ім`я було у "напарника" Шерлока Холмса?',
    options: [
      {
        text: "собака Баскервілів",
        isTrue: false
      },
      {
        text: "Артур Конан Дойл",
        isTrue: false
      },
      {
        text: "доктор Ватсон",
        isTrue: true
      },
      {
        text: "Місіс Хадсон",
        isTrue: false
      }
    ]
  },
  {
    text: 'Найпопулярніша гра з м`ячем',
    options: [
      {
        text: "Баскетбол",
        isTrue: false
      },
      {
        text: "Теніс",
        isTrue: false
      },
      {
        text: "Футбол",
        isTrue: true
      },
      {
        text: "Гандбол",
        isTrue: false
      }
    ]
  },
  {
    text: "Як називається портрет, написаний з самого себе?",
    options: [
      {
        text: "Автопортрет",
        isTrue: true
      },
      {
        text: "Самописка",
        isTrue: false
      },
      {
        text: "Дзеркальник",
        isTrue: false
      },
      {
        text: "Самопал",
        isTrue: false
      }
    ]
  },
  {
    text: "Що росте на городі?",
    options: [
      {
        text: "Цибуля",
        isTrue: true
      },
      {
        text: "Пістолет",
        isTrue: false
      },
      {
        text: "Кулемет",
        isTrue: false
      },
      {
        text: "Лук",
        isTrue: false
      }
    ]
  },
  {
    text: "Як називають мікроавтобуси, які здійснюють поїздки певними маршрутами?",
    options: [
      {
        text: "Рейсівка",
        isTrue: false
      },
      {
        text: "Путівка",
        isTrue: false
      },
      {
        text: "Курсовка",
        isTrue: false
      },
      {
        text: "Маршрутка",
        isTrue: true
      }
    ]
  },
  {
    text: "Відрізок що поєднує середини двох сторін трикутника",
    options: [
      {
        text: "Медіана",
        isTrue: false
      },
      {
        text: "Бісектриса",
        isTrue: false
      },
      {
        text: "Середня лінія",
        isTrue: true
      },
      {
        text: "Висота",
        isTrue: false
      }
    ]
  },
];